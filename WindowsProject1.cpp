...
// win32API는 GDI를 이용해서 그리기(렌더링) 작업을 함
// DC(백버퍼나 도화지같은 역할)에 출력하기 위하여 GDI 오브젝트를 사용
// GDI Object란?
// 그래픽 출력에 사용되는 도구
...
// 하지만 DE에서 사용중인 상태라면 해제가 불가능

// DC에 오브젝트 등록
// HGDIOBJ SelectObject(HDC hdc, HGDIOBJ obj)를 사용하여 오브젝트를 선택
// SelectObject를 사용하게 된다면 hdc에 obj를 선택시키고, 이전에 사용하고 있었던
// 같은 종류의 오브젝트를 반환해줌

// GDI 오브젝트 해제
// BOOL DeleteObject(HGDIOBJ obj)를 이용하여 오브젝트를 해제

// 스톡 오브젝트(Stock Object)란?
// 운영체제가 미리 만들어서 관리하는 GDI 오브젝트를 말합니다.
// 아주 빈번하게 사용되므로 운영체제가 부팅할 때부터 미리 만들어 놓습니다.
// 만들지 않아도 원한다면 언제든지 사용할 수 있으며, 운영체제가 관리하기 때문에
// 사용하고 난 후에 해제할 필요가 없습니다.

// HGDIOBJ GetStockObject(int fnObject)를 사용하여 스톡 오브젝트의 핸들러를 가져올 수 있음
// fnObject는 스톡 오브젝트의 미리 정의된 키 값을 전달
...
// Windows용 프로그램은 여러 개의 프로그램이 동시에 실행되는 멀티태스킹 시스템이며,
// 하나의 프로그램이 여러 번 실행될 수도 있습니다.
                     ...
                     _In_ int       nCmdShow)
{
    ...
    // 기본 메시지 루프입니다:
    // 발생한 이벤트를 메세지로 받아들여서 처리한다. (이벤트 드리븐 방식)

    // 잠깐 큐(Queue)에 대해
    // 선입선출(First In First Out) 형식의 자료구조 (먼저 들어간 데이터가 먼저 출력됨)

    // GetMessage() 가 하는 작업
    // 메세지 큐는 키보드나 마우스를 통해 발생하는 메세지를 저장하기 위해 할당된 큐 입니다.
    // 메세지 큐에서 메세지를 가져오고, 가져온 메세지를 메세지 큐에서 제거하며
    // 만약 가져온 메세지가 WM_QUIT 이라면 0 (FALSE)를 리턴하며,
    // 그 외의 메세지라면 1 (TRUE)를 리턴합니다.
    // 또, 에러 발생 시, -1을 리턴
    // -> 리턴 값을 통해 메세지 루프를 제어하기 위한 용도로 사용

    // GetMessage는 만약 메세지 큐에서 대기중인 처리되지 않은 메세지가 없다면
    ...
    // -> PeekMessage 사용 시, 처리할 메세지가 있다면 처리를 하되,
    ...
    {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) // 메세지가 들어왔을 시 메세지 처리
        {
            TranslateMessage(&msg);
            DispatchMessage(&msg);
        }
        else // 별도의 메세지가 없다면 대기 시간동안 다른 작업을 할 수 있으니 게임 연산이나 렌더 실행
        {
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(
       ...
       WIN_SIZE_X, WIN_SIZE_Y, // 창의 해상도 설정 (순서대로 너비와 높이, width height)
       // 부모 윈도우의 핸들을 지정(부모가 있다면)
       // 하나의 프로그램에서 여러개의 윈도우 창을 쓸 경우
       // 또는 하나의 프로그램 실행 시, 다중으로 열어 프로그램을 자동으로 실행시켜
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        {
            ...
            //// hdc에 방금 만든 색상의 브러쉬를 적용
            //// SelectObject가 반환하는 이전에 사용하고 있던 브러쉬를 oldBrush에 담음
            //HBRUSH oldBrush = (HBRUSH)SelectObject(hdc, myBrush);

            //Rectangle(hdc, 100, 100, 200, 200); // 크기와 위치 정보를 가지고 생성함
    ...
    case WM_DESTROY:
        ...
        break;
    default:
        // 위에서 처리하지 않은 메세지들은 기본적인 처리 방식으로 처리할 수 있도록 합니다.
        // 발생한 모든 메세지에 대한 처리를 할 수 없으므로 위의 케이스가 아닌 메세지에 대한
        // 처리를 알아서 하도록 합니다.
        return DefWindowProc(hWnd, message, wParam, lParam);
    }
    return 0;
